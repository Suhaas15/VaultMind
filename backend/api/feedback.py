...
class FeedbackSubmission(BaseModel):
    ...
    summary_quality: Optional[str] = "good"  # excellent/good/fair/poor

@router.post("/")
async def submit_feedback(feedback: FeedbackSubmission):
    """
    Submit doctor feedback on AI summary quality.
...
async def get_feedback_stats(days: int = 30):
    """
    Get aggregated feedback statistics showing system improvement.
    """
...
async def get_prompt_performance():
    ...
    Shows which prompts are performing best.
    """
    performance = metrics_service.get_prompt_performance()
    
    # Convert list to dict for easier consumption
...
async def get_improvement_trend(days: int = 30):
    """
    Get time-series data showing how the system has improved over time.
    """
...
async def evolve_prompts():
    """
    Trigger prompt evolution analysis.
    System will analyze performance and potentially promote better prompts.
    """
...
def _interpret_stats(stats: Dict[str, Any]) -> str:
    """Generate human-readable interpretation of stats."""
...
def _summarize_trend(trend: Dict[str, Any]) -> str:
    """Generate summary of improvement trend."""
